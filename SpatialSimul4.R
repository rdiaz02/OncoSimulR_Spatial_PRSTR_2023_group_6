...
SimulMigration <- function(grid, migrationProb = 0.5, 
    ...
    randomcoordinates <- function(grid, MigrationType = "Near"){
        ...
        # from the ones of the grid they belong to. It also ensures that only positive 
        # coordinates are selected for migrating cells.
    ...
    if ("" %in% grid$Genotype && TotalPopSize == grid$N[which(grid$Genotype == "")]){
        return (grid)
    } else {
        ...
        remote_probtest <- sample(c("Migration", "NoMigration"), 1, 
                                  prob = c(largeDistMigrationProb, 1 - largeDistMigrationProb))
        ## IF THERE IS ANY KIND KIND OF MIGRATION:
        if ((near_probtest == "Migration") || (remote_probtest == "Migration")) {
            finalpopcomp_mut <- grid[which((grid$Genotype != "") & (grid$N > 0)),]
            ...
            if (xor((near_probtest == "Migration"),(remote_probtest == "Migration"))) {
                ...
                if (near_probtest == "Migration"){
                    MigPopulationcomp$Coordinates <- randomcoordinates(grid = grid, "Near")
                } else if (remote_probtest == "Migration"){
                    MigPopulationcomp$Coordinates <- randomcoordinates(grid = grid, "Remote")
                }
            } else if ((near_probtest == "Migration" & remote_probtest == "Migration")) {
                # When both migration probabilities are satisfied for a grid, the 
                # the number of cells that migrate to near and remote places are
                # randomly determined (at least one of the migrating cells must go 
                # to each site).
                NearMigPop_number <- sample (from = 1, to = MigPopulation - 1, 1)
                ...
                NearMigPopulationcomp$Coordinates <- randomcoordinates(grid = grid, "Near")
                RemoteMigPopulationcomp$Coordinates <- randomcoordinates(grid = grid, "Remote")
        ...
        return (list(grid, MigPopulationcomp))
              ...
              # The original grid is returned without any change in genotype populations 
...
AddWT_newgrid <- function(grid, initSize) {
  # When a new grid is created, it initializes with initSize wild-type cells plus
  # the arriving mutant cells. Already existing demes will have an empty string
  # in one of the genotypes, indicating the WT (although the WT population could be 0).
  if (! "" %in% grid$Genotype) {
    new_grid <- rbind(grid, data.frame(Genotype = "",
                                       N = initSize,
                                       Coordinates = I(list(grid$Coordinates[[1]]))))
    return (new_grid)
  } else {
    return (grid)
  }
...
SpatialOncoSimul <- function(fp, model = "Exp", 
                             ...
                             maxMigrationPercentage = 0.2,
                             SpatialModel = "3D", 
                             spatialDemesMax = -1, spatialIterMax = 20,
    ...
    oncoSimulIndiv_grid <- function(grid){
        ...
        if (sum(grid$N) > (exp(1) - 1)){
            ...
            simul_grid <- data.frame(Genotype = simul_grid$GenotypesLabels,
                                     N = simul_grid$pops.by.time[nrow(simul_grid$pops.by.time), -1],
                                     Coordinates = I(rep(list(grid$Coordinates[[1]]), 
                                                         length(simul_grid$GenotypesLabels))))
          
            # After running OncoSimulIndiv over a grid, if not wild-type genotypes are 
            # in the results, we add them with a population number of 0. This will
            # let us to distiguish between old grids and newly created grids in the 
            # migration phase.
    ...
    while (iter <= spatialIterMax) {
      ...
      # during specific time units.
        if (length(finalgrid_list) == 0) {
            ...
            simul_grid <- data.frame(Genotype = simul_grid$GenotypesLabels, 
                                     N = simul_grid$pops.by.time[nrow(simul_grid$pops.by.time), -1])
            if (SpatialModel == "1D"){
                  simul_grid$Coordinates <- rep(list(0), length(simul_grid$Genotype))
                                                
            } else if (SpatialModel == "2D"){
                  simul_grid$Coordinates <- rep(list(c(0,0)), length(simul_grid$Genotype))
            } else if (SpatialModel == "3D"){
                  simul_grid$Coordinates <- rep(list(c(0,0,0)), length(simul_grid$Genotype))
            }
      ...
      if (iter < spatialIterMax){
          ...
          if (SpatialModel == "1D"){
              ...
              # sum(initSize) must be greater than (exp(1)-1) to calculate K initial 
              # population equilibrium size. So all coordinates (grids) that only contain
              # one cell are deleted from the dataframe.
              join_grid <- join_grid[,c("Genotype", "N", "Coordinates")]
              grid_list <- split(join_grid, f = list(join_grid$Coordinates, drop = TRUE))
              conv_to_list <- function(x) {x$Coordinates <- rep(list(x$Coordinates[1]), 
                                                                  length(x$Genotype)) 
                                            return (x)}
              grid_list <- mclapply(grid_list, function(x) conv_to_list(x), mc.cores = mc.cores)
          } else if (SpatialModel == "2D") {
              ...
              join_grid <- aggregate(N ~ Genotype + Coordinate_x + Coordinate_y, 
                                     data = migration_out_df, FUN = sum)
              Coordinates <- matrix(c(join_grid$Coordinate_x, join_grid$Coordinate_y), 
                                    ncol = 2)
              join_grid$Coordinates <- split(Coordinates, row(Coordinates))
              grid_list <- split(join_grid, f = list(join_grid$Coordinate_x,
                                                     join_grid$Coordinate_y), drop = TRUE)
              grid_list <- mclapply(grid_list, function(x) x[,-c(2,3)], mc.cores = mc.cores)
          } else if (SpatialModel == "3D") {
              coord_matrix <- do.call(rbind, migration_out_df$Coordinates)
              colnames(coord_matrix) <- c("Coordinate_x", "Coordinate_y", "Coordinate_z")
              migration_out_df <- cbind(migration_out_df, coord_matrix)
              join_grid <- aggregate(N ~ Genotype + Coordinate_x + Coordinate_y + 
                                         Coordinate_z, data = migration_out_df, FUN = sum)
              Coordinates <- matrix(c(join_grid$Coordinate_x, join_grid$Coordinate_y, 
                                      join_grid$Coordinate_z), ncol = 3)
              join_grid$Coordinates <- split(Coordinates, row(Coordinates))
              grid_list <- split(join_grid, f = list(join_grid$Coordinate_x,
                                                     join_grid$Coordinate_y,
                                                     join_grid$Coordinate_z), drop = TRUE)
              grid_list <- mclapply(grid_list, function(x) x[,-c(2:4)], mc.cores = mc.cores)
          }
      ...
      print(finalgrid_list)
      print(iter)
      iter <- iter + 1 
    }
    return (finalgrid_list)
}

##Añadir las columnas con cada coordinada y luego borrarlas.
...
z <- SpatialOncoSimul(fp = fe,
                      ...
                      errorHitWallTime = FALSE, initMutant = c("i"), spatialIterMax = 10, SpatialModel = "3D")


to_SpatialOncoSimul <- function(x, ...){
  UseMethod("to_SpatialOncoSimul")
}
## Take a list and return (if possible) a SpatialOncoSimul object.

to_SpatialOncoSimul.list <- function (finalgrid_list, ...){
    attributes <- c("FinalPopulation", "TotalGrids", "TotalIter",
                  "GenotypeLabels", "PopulationPerGenotype", "GridPerGenotype",
                  "WinGrids", "ListofGrids")
    grids_dataframe <- bind_rows(finalgrid_list)
    FinalPopulation <- sum(grids_dataframe$N)
    TotalGrids <- length(finalgrid_list)
    TotalIter <- spatialIterMax -1 
    PopulationPerGenotype <- aggregate(N ~ Genotype, data = grids_dataframe, FUN = sum)
    GenotypeLabels <- PopulationPerGenotype$Genotype
    GridPerGenotype <- c()
    for (Genotype in GenotypeLabels){
        counts <- nrow(grids_dataframe[which(grids_dataframe$Genotype == Genotype
                                             & grids_dataframe$N != 0), ])
        GridPerGenotype <- c(GridPerGenotype, counts)
    }

    GridPerGenotype <- data.frame(Genotype = GenotypeLabels, GridNum = GridPerGenotype) 
    WinGrids <- c()
    maxpergrid <- unlist(mclapply(finalgrid_list, function (x) (x$Genotype[which(x$N == max(x$N))]))) 
    for (Genotype in GenotypeLabels){
      counts <- length(maxpergrid[which(maxpergrid == Genotype)])
      WinGrids <- c(WinGrids, counts)
    }
    ListofGrids <- finalgrid_list
    tmp <- list (FinalPopulation, TotalGrids, TotalIter,
                 GenotypeLabels, PopulationPerGenotype, GridPerGenotype,
                 WinGrids, ListofGrids)
    class(tmp) <- c("SpatialOncoSimul")
    return (tmp)
}
...
# distintos (solo puede haber un valor para init size, porque después de la migración
# a cada nuevo grid se le tiene que añadir esa cantidad de células no tumorales.)
library(plyr)
ddply(data, .(A,B), summarise, test = list(C))
## Test que el número de coordenadas en el grid que entra en SimulMigration 
# coincide con el el SpatialModel seleccionado.


df$join_grid<- mcapply(list(join_grid), 1, function(x) paste(x[!is.na(x) & x != "No"], collapse = ", "));
df;