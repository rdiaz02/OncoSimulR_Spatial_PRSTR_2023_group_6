...
SpatialOncoSimul <- function(fp, model = "Exp", 
  ...
  oncoSimulIndiv_grid <- function(grid){
    ...
    if (sum(grid$N) > (exp(1) - 1)){
      ...
      # let us to distinguish between old grids and newly created grids in the 
      # migration phase.
  ...
  #return (c(FinalObject, finalgrid_list))
  return (FinalObject)
}
...
test_that("initSize and initMutant are not length 1", {
  fe <- allFitnessEffects(
    data.frame(parent = c("Root", "Root", "i"),
               child = c("u" , "i" , "v"),
               s = c(0.1 , -0.05 , 0.25),
  ...
  expect_error(SpatialOncoSimul(fp = fe,
                                ...
                                spatialIterMax = 10, SpatialModel = "3D"),
               "Argument 3 must have names.", ignore.case = TRUE)
})
...
test_that("SpatialOncoSimul return a SpatialOncosimul object", {
  ...
  p <- SpatialOncoSimul(fp = fe,
                        ...
                        errorHitMaxTries = FALSE,
                        errorHitWallTime = FALSE, initMutant = c("i"), 
                        spatialIterMax = 3, SpatialModel = "3D")
